...
def main_cifar_dir(dataset, algo):
    ...
    for dir_alpha in [10.0, 1.0, 0.5, 0.1]:
        for j in range(hypers["cnt"]):
            ...
            para_dict["lr"] = lr
            para_dict["n_clients"] = 30 #总客户端数量
            para_dict["c_ratio"] = 0.1 #挑选因子
            para_dict["local_epochs"] = 5
            para_dict["max_round"] = 50 #一次训练计划执行的最大轮次，每执行一次训练计划dir_alpha，输出一次log文件
            para_dict["test_round"] = 1 #测试集测试轮次，每一轮保存一次全局模型
